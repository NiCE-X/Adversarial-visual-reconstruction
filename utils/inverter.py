...
class StyleGANInverter(object):
  ...
  def invert(self, image, label, num_viz=0):
    ...
    for step in pbar:
      ...
      out_recs = []
      out_oris = []
      out_rec = starG(resize(x_rec), label[0])
      out_ori = starG(resize(x), label[0])
      ...
      if step == self.iteration:
          for num in range(len(out_recs)):
              stargan_results.append(0.5*255.*(out_recs[num][0]+1).detach().cpu().numpy().transpose(1,2,0))
          for num in range(len(out_oris)):
              stargan_results.append(0.5*255.*(out_oris[num][0]+1).detach().cpu().numpy().transpose(1,2,0))
